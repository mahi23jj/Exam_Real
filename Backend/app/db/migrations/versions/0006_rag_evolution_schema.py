...
def upgrade() -> None:
    ...
    # ------------------------------------------------------------------
    # 7. Trigger (split into TWO execute calls for asyncpg)
    # ------------------------------------------------------------------
    op.execute("""
...
def downgrade() -> None:
    ...
    op.execute("""
        DROP FUNCTION IF EXISTS content_blocks_tsv_trigger()
    """)

    op.execute("""
        DROP INDEX IF EXISTS ix_content_blocks_content_tsv_gin
    """)
