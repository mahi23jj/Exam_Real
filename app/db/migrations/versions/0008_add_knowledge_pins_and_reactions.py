...
def upgrade() -> None:
    ...
    pintype_enum = postgresql.ENUM(
        'MEMORY_TRICK', 'EXPLANATION', 'EXAM_TIP', 'WARNING',
        'COMMON_MISTAKE', 'IMPLEMENTATION_TIP', 'FORMULA_TIP', 'OTHER',
        name='pintype'
    )
    ...
    targettype_enum = postgresql.ENUM(
        'PIN', 'QUESTION', 'REPLY', 'COURSE', 'USER', 'TOPIC',
        name='targettype'
    )
    targettype_enum.create(op.get_bind(), checkfirst=True)

    locationtargettype_enum = postgresql.ENUM(
        'SENTENCE', 'PARAGRAPH', 'HEADING', 'DIAGRAM', 'IMAGE', 'EXAM_QUESTION', 'PAGE',
    ...
    op.create_table(
        ...
        sa.Column('target_type', locationtargettype_enum, nullable=False, server_default='PARAGRAPH'),
        ...
        sa.Column('pin_type', pintype_enum, nullable=False, server_default='EXPLANATION'),
    ...
    op.create_index('ix_knowledge_pins_document_version', 'knowledge_pins', ['document_version'])
    ...
    op.create_table(
        ...
        sa.Column('reaction_type', reactiontype_enum, nullable=False, server_default='LIKE'),
        sa.Column('created_at', sa.DateTime(), nullable=False),
        sa.Column('updated_at', sa.DateTime(), nullable=False),
    )
